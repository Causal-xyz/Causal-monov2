...
function CreateProposalContent() {
  ...
  // After successful creation, read the latest proposal address for redirect
  const { proposalAddress: newProposalAddress } = useLatestProposalAddress(
    factoryAddress,
    isSuccess,
  );
  ...
  useTransactionToast({
    ...
    error,
    labels: {
      success: "Proposal created! Redirecting...",
      pending: "Deploying proposal...",
    },
  });
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 text-center sm:px-6">
        ...
        <p className="mb-2 text-muted-foreground">
          Your proposal has been deployed. Redirecting to set up AMM pools...
        </p>
        <Loader2 className="mx-auto h-6 w-6 animate-spin text-causal" />
        <p className="mt-4 text-xs text-muted-foreground">
          If you are not redirected,{" "}
          <button
            onClick={() => router.push("/proposals")}
            className="text-causal underline"
          >
            view all proposals
          </button>
        </p>
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12 sm:px-6">
      ...
      <div className="mb-6 rounded-lg border border-causal/30 bg-causal/5 p-4 text-sm text-muted-foreground">
        <strong className="text-foreground">Two-step process:</strong>{" "}
        First create the proposal, then set up AMM pools on the proposal detail page.
        This keeps each transaction within gas limits.
      </div>
